...
export function PecaForm({ open, onOpenChange, peca, contatos, date, pending, onSubmit }: { open: boolean; onOpenChange: (open: boolean) => void; peca?: Row<"pecas">; contatos: Row<"contatos">[]; date?: string; pending: boolean; onSubmit: (value: Insert<"pecas">) => void }) {
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); const status = formValue(data, "status") || "producao"; onSubmit({ contato_id: formValue(data, "contato_id"), descricao: formValue(data, "descricao") || null, data_deixou: formValue(data, "data_deixou") || null, estimativa: formValue(data, "estimativa") || "—", status, data_pronta: status === "pronta" ? (peca?.data_pronta ?? localDateIso()) : peca?.data_pronta }); }
